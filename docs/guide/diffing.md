...
Containers are compared recursively when **both sides** are container-like (duck-typed, so third-party proxies such as observ's reactive objects work too):
...
Insertions at the end use the JSON pointer `-` token (RFC 6901 for "append"):
...
Sets have no indices, so patchdiff extends JSON patch slightly: an element is **added** with the `-` token (like a list append) and **removed** by addressing the element itself as the final path token:
...
The second list `diff` returns is not just the first with `add`/`remove` swapped — the operations are also **ordered for reverse application**, so indices resolve correctly as each patch is applied. Always apply `reverse_ops` as-is, in order:
